...
import { uploadVideo } from 'src/apis/file.api'
import { addNewSession, updateSession } from 'src/apis/session.api'
import Input from 'src/components/Input'
import { SessionCreateSchema, SessionUpdateSchema, sessionCreateSchema } from 'src/utils/rules'

type PropsType = {
  handleFormDisplay: (status: boolean, session: SessionUpdateSchema | null) => () => void
  sessionUpdate: SessionUpdateSchema | null
  lessonId: number
}

export default function SessionForm({ handleFormDisplay, sessionUpdate, lessonId }: PropsType) {
  ...
  const {
    ...
    setValue,
    formState: { errors }
  } = useForm<SessionUpdateSchema | SessionCreateSchema>({
    resolver: yupResolver(sessionCreateSchema)
  })

  useEffect(() => {
    if (sessionUpdate) {
      setValue('id', sessionUpdate.id)
      setValue('sessionName', sessionUpdate.sessionName)
      setValue('videoAddress', sessionUpdate.videoAddress)
      setValue('status', sessionUpdate.status)
    }
    setValue('lessonId', lessonId)
  }, [setValue, sessionUpdate, lessonId])

  const uploadVideoMutation = useMutation({
    mutationFn: (body: FormData) => uploadVideo(body)
  })

  const updateSessionMutation = useMutation({
    mutationFn: (body: SessionUpdateSchema) => updateSession(body)
  })

  const createSessionMutation = useMutation({
    mutationFn: (body: SessionCreateSchema) => addNewSession(body)
  })

  const handleVideoFileInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setVideoFile(event.target.files ? event.target.files[0] : null)
  }
  ...
  const handleUploadVideo = () => {
    if (videoFile) {
      ...
      uploadVideoMutation.mutate(formData, {
        onSuccess(data) {
          setIsUploaded(true)
          setValue('videoAddress', data.data.data?.filename as string)
  ...
  const handleFormSubmit = handleSubmit((data) => {
    if (sessionUpdate) {
      updateSessionMutation.mutate(data as SessionUpdateSchema, {
        onSuccess(data) {
          toast.success(data.data.message)
      ...
      <div className='relative w-full h-full max-w-2xl px-4 md:h-auto'>
        <div className='relative bg-white rounded-lg shadow dark:bg-gray-800'>
          <div className='flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700'>
            <h3 className='text-xl font-semibold dark:text-white'>{sessionUpdate ? 'Update' : 'Add'} session</h3>
            <button
              type='button'
          ...
          <div className='p-6 space-y-6'>
            <form>
              <div className='grid grid-cols-6 gap-6'>
                ...
                <div className='col-span-6 sm:col-span-3'>
                  <label className='block mb-2 text-sm font-medium text-gray-900 dark:text-white' htmlFor='file_input'>
                    {isUploaded ? 'Uploaded' : 'Not Upload'}
                  </label>
                  <button
                    type='button'
