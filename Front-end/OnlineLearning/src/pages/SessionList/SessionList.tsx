...
import { SessionUpdateSchema } from 'src/utils/rules'

export default function SessionList() {
  const { lessonId } = useParams()
  const [isFormDisplay, setIsFormDisplay] = useState<boolean>(false)
  const [sessionUpdate, setSessionUpdate] = useState<SessionUpdateSchema | null>(null)

  const isIdValid = lessonId !== undefined && /^[\d+]+$/.test(lessonId)

  const { data } = useQuery({
    queryKey: ['session', lessonId],
    queryFn: () => (isIdValid ? getSessionsByLesson(Number(lessonId)) : Promise.resolve(null))
  })

  const handleFormDisplay = (status: boolean, session: SessionUpdateSchema | null) => () => {
                  ...
                  <tbody className='bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700'>
                    {sessionData &&
                      sessionData.map((session) => (
                        <tr className='hover:bg-gray-100 dark:hover:bg-gray-700' key={session.id}>
                          ...
                          <td className='p-4 space-x-2 whitespace-nowrap'>
                            <button
                              ...
                              data-drawer-placement='right'
                              onClick={handleFormDisplay(true, { ...session, lessonId: Number(lessonId) })}
