...
export default function Profile() {
  ...
  const editProfileMutation = useMutation({
    mutationFn: (body: User) => editUserProfile(username as string, body)
  })
  const {
    register,
    handleSubmit,
  ...
  }, [username, navigate, setValue, profileFromLS])

  const handleChangeGeneralInfo = handleSubmit((data) => {
    const newProfile: User = { ...data, id: profileFromLS.id }
    ...
    <div className='grid grid-cols-1 px-4 pt-6 xl:grid-cols-3 xl:gap-4 dark:bg-gray-900 mt-20 ml-20 mr-20'>
      ...
      <div className='col-span-2'>
        <div>
          ...
          <ChangePassword username={username as string} />
        </div>
      </div>
    </div>
  )
